...
const RightSidebar = memo(
  ({
    ...
    return (
      <div
        className={`w-[22vw] fixed right-0 top-0 z-10 hidden lg:block ml-auto p-6 ${
          isDarkMode ? "text-black" : "text-gray-900"
        } transition-all duration-500`}
      >
        <div
          className={`flex items-center p-3 shadow-sm rounded-lg mb-6 ${
            isDarkMode ? "text-white bg-zinc-800" : "text-gray-900"
          } transition-all duration-500`}
        >
          {user ? (
            <Link
        ...
        <div
          className={`bg-white p-4 shadow-sm rounded-lg mb-6 ${
            isDarkMode ? "text-white bg-zinc-800" : "text-gray-900"
          } transition-all duration-500`}
        >
          <h4 className="font-semibold mb-3">Suggestions for you</h4>
            ...
            {loading ? (
              ...
              suggestedUsers.map((suggestedUser) => (
                ...
                >
                  ...
                  {!user.following.includes(suggestedUser._id) ? (
                    <button
                      onClick={() => handleSendFollowRequest(suggestedUser._id)}
